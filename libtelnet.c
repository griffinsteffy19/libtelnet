                                ...
                                char us, char him)
{
    ...
    if(telnet->q_cnt >= telnet->q_size)
    {
        ...
        if((qtmp = (telnet_rfc1143_t *)realloc(telnet->q,
                                               sizeof(telnet_rfc1143_t) * (telnet->q_size + Q_BUFFER_GROWTH_QUANTUM)))
           == 0)
        {
                           ...
                           char * buffer, size_t size)
{
    ...
    if((unsigned)buffer[0] != TELNET_ENVIRON_SEND && (unsigned)buffer[0] != TELNET_ENVIRON_IS && (unsigned)buffer[0] != TELNET_ENVIRON_INFO)
    {
        _error(telnet, __LINE__, __func__, TELNET_EPROTOCOL, 0,
    ...
    for(index = 0; index != count; ++index)
    {
        ...
        while(c < buffer + size)
        {
            ...
            if((unsigned)*c == TELNET_ENVIRON_VAR || (unsigned)*c == TELNET_ENVIRON_VALUE || (unsigned)*c == TELNET_ENVIRON_USERVAR)
            {
                break;
            }
...
static void _process(telnet_t * telnet, const char * buffer, size_t size)
{
    ...
    for(i = start = 0; i != size; ++i)
    {
        ...
        // printf("p[%02X]", byte, telnet->state);
        switch(telnet->state)
        {
            /* regular data */
			 ...
			 * switch states */
                ...
                else
                {
                    if(i != start)
                    {
                        ev.type        = TELNET_EV_DATA_PRMPT;
                        ev.data.buffer = buffer + start;
                        ev.data.size   = i - start + 1;
                        telnet->eh(telnet, &ev, telnet->ud);
                    }
                }
                break;
